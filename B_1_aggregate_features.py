# -*- coding: utf-8 -*-
"""
B_1_aggregate_features.py  ── [분석 단계] 원자료에서 N년 창으로 지표 집계 (API 없음)
...
[출력]  (기존 B_2_train_model.py / B_3_predict.py 와 100% 동일한 형식·컬럼명)
  ...
  - data/features.csv, data/predict_features.csv
    → B_2_train_model.py, B_3_predict.py 를 수정 없이 그대로 재사용 가능

[사용법]
  python B_1_aggregate_features.py            # 기본 3년 창 + 정규 파일 승격
  python B_1_aggregate_features.py 5          # 5년 창
  python B_1_aggregate_features.py 3 --no-promote   # features_3y.csv 만 만들고 정규파일은 유지
...
def aggregate(years=3, promote=True):
    if not os.path.exists(RAW_PATH):
        sys.exit(f"[에러] {RAW_PATH} 가 없습니다. 먼저 A_3_collect_raw_disclosures.py 를 실행하세요.")
    ...
    if promote:
        ...
        print("  → 이제 B_2_train_model.py, B_3_predict.py 를 그대로 실행하면 됩니다.")
