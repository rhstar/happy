"""
build_network_groups.py — 위험기업 간 공유 임원 네트워크 클러스터 산출

network_executives_all.csv(2013~ 위험기업 임원)에서, 실질 경영진을 공유하는
위험기업들을 연결해 클러스터(그룹)를 만든다. 사외이사·감사는 제외.

실행 위치 무관. 출력: network_analysis/data/network_groups.csv
"""
import pandas as pd
...
pd.DataFrame(list(company_to_group.items()), columns=['위험기업', '그룹']).to_csv(
    os.path.join(ROOT, "network_analysis/data/network_groups.csv"),
