...
조회 대시보드와 네트워크 분석이 하나의 통합 앱(한 포트)으로 합쳐졌으므로,
링크/포트 하나(http://localhost:8501)로 두 기능을 모두 사용한다.
...
REQUIRED = [
    ...
    "network_analysis/data/network_executives_all.csv",
    "network_analysis/data/network_groups.csv",
...
# 있으면 포함(없어도 앱은 폴백 동작). 4_predict.py 실행 시 생성됨.
OPTIONAL = [
...
def main():
    ...
    if "data/risk_ranking_all.csv" not in opt_ok:
        ...
        print("       전체 상장사(대조군 포함) 점수를 넣으려면 먼저 'python 4_predict.py'를 실행하세요.")
    if missing:
        print(f"\n누락(필수): {len(missing)}개")
        for f in missing:
            print(f"  --  {f}")
...
def _write_batch(out):
    ...
    run = (
        ...
        "echo 왼쪽 사이드바에서 '조회 대시보드'와 '네트워크 분석'을 전환하세요.\n"
...
def _write_readme(out):
    txt = (
        ...
        "조회 대시보드와 네트워크 분석이 하나의 앱으로 통합되어 있습니다.\n\n"
        "----------------------------------------\n"
        "[전제 조건]\n"
        "----------------------------------------\n"
        ...
        "   - 왼쪽 사이드바에서 두 기능을 전환합니다:\n"
        "       · 조회 대시보드 : 종목 검색 -> 위험도, 순위, 위험 경영진\n"
